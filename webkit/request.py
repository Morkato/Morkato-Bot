from typing import Optional, Literal, TypeVar, Union, Any, overload
...
def resolve_body(body: Any) -> tuple[int, str, bytes]:
  if isinstance(body, ( bytes, bytearray )):
    ...
    (length, type, content) = resolve_body(json.dumps(body))

    return (
      length,
      "application/json" if not isinstance(content, str) else "text/plain",
      content
    )
...
class URL:
  ...
  def __init__(self, route: Optional[Union[URLRoute, str, None]] = None, *, url: Optional[Union[str, yarl.URL]] = None, parameters: Optional[URLParameters] = None) -> None:
...
class Request:
  def __init__(self, method: Methods, url: Optional[Union[URL, str]] = None, *, headers: Optional[Headers] = None, body: Optional[Any] = None) -> None:
    ...
    if not body is None:
      (length, content_type, content) = resolve_body(body)

      self.__headers.set('content-length', length)
      self.__headers.set('content-type', content_type)

      self.__body = content

  def __repr__(self) -> str:
    return f'{self.__method} {self.__uri.route}'
