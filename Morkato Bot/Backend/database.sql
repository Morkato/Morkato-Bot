...
CREATE TABLE arts (
  ...
  CONSTRAINT check_name CHECK (name ~ '^\D+$'),
  ...
  CONSTRAINT check_embed_title CHECK (
	  embed_title IS NULL
	  OR (LENGTH(embed_title) <= 96 AND LENGTH(embed_title) > 1 AND embed_title ~ '^\D.+$')
  ),
  CONSTRAINT check_embed_description CHECK (
	  embed_description IS NULL
	  OR (LENGTH(embed_description) <= 4096 AND LENGTH(embed_description) > 1 AND embed_description ~ '^\D.+$')
  ),
