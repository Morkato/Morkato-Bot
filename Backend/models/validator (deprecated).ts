...
export default function validator<T extends any = {}>(obj: any, keys: DynamicKeyValue<ChoiceKeyOption>): T {
  ...
  const filteredKeys = utils.object.map(keys, ([key, val]) => val === 0 || val === 1 ? { option: val } : val)
  
  const schema = Joi.object(utils.object.map(filteredKeys, ([key, value]) => validators[key](value))).required().min(1)
