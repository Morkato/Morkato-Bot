...
ALTER TABLE arts ADD CONSTRAINT check_name CHECK (LENGTH(name) > 1 AND LENGTH(name) < 132 AND name ~ '^[^\d \n\t\v\r\f\b]+$');
...
ALTER TABLE attacks ADD CONSTRAINT check_name CHECK (LENGTH(name) > 1 AND LENGTH(name) < 132 AND name ~ '^[^\d \n\t\v\r\f\b]+$');
